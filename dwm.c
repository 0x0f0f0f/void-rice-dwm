...
int
xresource_load(XrmDatabase db, char *name, enum resource_type rtype, void *dst)
{
	char **sdst = dst;
	int *idst = dst;
	float *fdst = dst;
	...
	snprintf(fullclass, sizeof(fullclass), "%s.%s", "Dwm", name);
...
setup(void)
{
    ...
    /* Here you change the dwm bar size */
